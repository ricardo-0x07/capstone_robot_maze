...
class Robot(object):
    ...
    def next_move(self, sensors):
        ...
        print 'self.new_path_previous_state', self.new_path_previous_state
        # print 'self.path',self.path
        # print 'self.previous_path', self.previous_path
        ...
        if self.robot_pos['location'][0] == self.new_path_previous_state[0][0] and self.robot_pos['location'][1] == self.new_path_previous_state[0][1] and self.robot_pos['heading'] == self.new_path_previous_state[1]:
            ...
            # self.current_state = self.previous_path.pop(len(self.previous_path)-1)
            # action = self.current_state[2]
            # reverse_action1 = None
            # reverse_action2 = None
            # action_list = []
            # reverse_action1 = (0, action[1]*-1)
            ...
            # action_list = [reverse_action2, reverse_action1]
            # self.reverse_actions.extend(action_list)
            # self.reverse_actions = action_list
            action = self.reverse()
            print 'self.robot_pos', self.robot_pos
            rotation = action[0]
            movement = action[1]
            print 'reverse action', action
            print ' '
            return rotation, movement
    ...
    def get_next_state(self):
        path = tuple()
        sorted_frontier = sorted(self.frontier,key=len)
    ...
    def update(self, sensors, path, state):
        ...
        #     self.actions = [action[2] for action in path]
        #     self.re_init()
        #     return 'Reset', 'Reset'
        ...
        # print 'path', path
        print 'state', state
        print 'self.successors', self.successors
    ...
    def isGoalState(self, location):
        if location[0] in self.goal_bounds and location[1] in self.goal_bounds:
            print 'isGoalState location', location
            return True
        else:
            return False
